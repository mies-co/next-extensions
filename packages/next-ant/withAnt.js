...
const getAppConfigWithAnt = (customAppConfig) => {
  ...
  if (!styleFileExists) {
    console.log(
      "\x1b[31m%s\x1b[0m",
      `Ant theme style file not found.\nFor next-ant to work, please create the following file: ${path.resolve(
        appConfig.ant.theme
      )}`
    );
  }
          ...
          if (t.includes("scss") || t.includes("sass") || t.includes("less")) {
            if (Array.isArray(rule.use))
              rule.use = rule.use.map((u) => {
                if (u === "sass-loader" || u === "less-loader")
                  u = AntdScssThemePlugin.themify(u);
                else if (
                  typeof u === "object" &&
                  (u.loader === "sass-loader" || u.loader === "less-loader")
                )
                  u = AntdScssThemePlugin.themify(u);
                return u;
              });
